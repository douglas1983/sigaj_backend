...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
  ...
  private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
    ...
    if (token != null) {
      ...
      String iduser = JWT.require(Algorithm.HMAC512(SECRET.getBytes())).build().verify(token.replace(TOKEN_PREFIX, ""))
          .getIssuer();

      if (user != null) {
        return new UsernamePasswordAuthenticationToken(user, iduser, new ArrayList<>());
      }
      return null;
    }
    return null;
